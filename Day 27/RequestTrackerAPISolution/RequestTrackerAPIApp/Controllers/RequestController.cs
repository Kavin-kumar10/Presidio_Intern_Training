...
namespace RequestTrackerAPIApp.Controllers
{
    ...
    public class RequestController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(Request),StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ErrorModel),StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Request>> RaiseRequest(GetRequestDTO getrequestdto)
        {
            try
            {
                Request request = new Request()
                {
                      RequestMessage = getrequestdto.RequestMsg,
                      RequestRaisedBy = getrequestdto.RaisedById,
                };
